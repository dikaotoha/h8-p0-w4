function cariModus(arr) {
    ...
    var max = jumlahModus[0];
    var indexMax = 0;
    var cekModus = [];

    for (k = 0; k < jumlahModus.length; k++) {
        // console.log('loop k');
        if (max < jumlahModus[k]) {
            max = jumlahModus[k];
            indexMax = k;
        }
        if (jumlahModus[k] >= 2) {
            cekModus.push(jumlahModus[k]);
            if (cekModus > 0) {
                if (jumlahModus.length < 2) {
                    return -1;
                }
                return modus[indexMax]
            }
        }
